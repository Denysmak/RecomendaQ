...
def preprocess_data(df):
    ...
    # Obter a taxa de câmbio atual para ajustar os preços para a moeda local (BRL)
    current_exchange_rate = get_exchange_rate(pd.Timestamp.now())

    # Ajustar os preços usando a taxa de câmbio atual
    df['Price'] = df['Price'] * current_exchange_rate
...
def get_exchange_rate(date):
    ...
    try:
        exchange_rate = currency_rates.get_rate(base_currency, target_currency, date)
        return exchange_rate
    except Exception as e:
        print(f"Erro ao obter taxa de câmbio: {e}")
        return 1.0  # Retornar 1.0 se houver erro para não afetar o preço
...
def process_artists(data):
    if isinstance(data, pd.DataFrame):
        ...
        raise ValueError("Os dados de entrada devem ser um DataFrame ou um dicionário.")
...
def display_metrics(model_name, y_test, predictions, class_labels, inverse_mapping):
    ...
    precision = precision_score(
        y_test, predictions, average='weighted', zero_division=1)
    ...
    inverse_predictions = [inverse_mapping.get(label, 'Classe Desconhecida') if label != -1.0 else 'Classe Desconhecida' for label in predictions]
    ...
    all_labels = set(predictions).union(set(y_test))
...
def display_majority_percentage(y_test, predictions, inverse_mapping):
    # Mapear as previsões de volta para os nomes correspondentes
    inverse_predictions = [inverse_mapping.get(label, "Classe Desconhecida") for label in predictions]

    # Criar DataFrame com as previsões mapeadas
    predictions_df = pd.DataFrame({
        'True Label': y_test,
        'Predicted Label': predictions,
        'Mapped Prediction': [inverse_mapping.get(label, 'Classe Desconhecida') if label != -1.0 else 'Valor Especial' for label in predictions]
    })
    ...
    st.write(f'A classe com maior porcentagem é "{majority_class}" com {majority_percentage:.2f}% de chance.')
